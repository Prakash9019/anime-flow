...
const handlePickFile = async () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Bulk edit complete', [{ text: 'OK', onPress: () => navigation.goBack() }]);
      setFile(null);
    } catch (e) {
      Alert.alert('Bulk edit failed', 'Could not upload file.');
    }
    setUploading(false);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Bulk Edit</Text>
      </View>
      <ScrollView style={styles.content}>
        <Text style={styles.instructions}>
          1. Download the csv file and fill with proper data.{'\n'}
          2. After editing, upload and submit below.{'\n'}
          3. For multiple options, separate items with a comma.
        </Text>
        <TouchableOpacity
